...
const handleCreatorAccess = async (data) => {
  setSubmitting(true); // Prevent double submit
  ...
  try {
    ...
    setSubmitting(false);
    setLoading(false); // CRITICAL: This was missing!
  }
};
  ...
  className={`w-full py-2 font-semibold rounded-full transition 
    ${hasApplied || submitting 
      ? "bg-gray-400 cursor-not-allowed" 
      : "bg-emerald-600 hover:bg-emerald-700 text-white"}`}
>
  {submitting ? "Submitting..." : hasApplied ? "Already Applied" : "Apply For Creator Role"}
</button>
